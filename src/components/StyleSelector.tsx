...
const StyleSelector = ({value, onChange, isOpen, setIsOpen}: {value: ThumbnailStyle; onChange: (style: ThumbnailStyle) => void; isOpen: boolean; setIsOpen: (isOpen: boolean) => void}) => {
  ...
  return (
    <div className='space-y-3 relative'>
      ...
      <div>
        <button type='button' onClick={() => setIsOpen(!isOpen)} className='w-full flex items-center justify-between p-3 bg-zinc-800 border border-zinc-700 rounded-md text-left text-zinc-200'>
            <div className='space-y-1'>
                <div>
                     {styleIcons[value]}
                    <span className='ml-2 text-zinc-200 font-medium'>{value}</span>
                </div>
        ...
        {isOpen && (
            <div className='absolute z-10 mt-1 w-full bg-zinc-900 border border-zinc-700 rounded-md shadow-lg max-h-60 overflow-y-auto'>
                {Object.keys(styleDescriptions).map((style) => (
                    <div key={style} onClick={() => {onChange(style as ThumbnailStyle); setIsOpen(false);}} className='flex items-center p-3 hover:bg-zinc-800 cursor-pointer'>
                        {styleIcons[style as ThumbnailStyle]}
                        <div className='ml-2'>
                            <div className='text-zinc-200 font-medium'>{style}</div>
                            <div className='text-xs text-zinc-400'>{styleDescriptions[style as ThumbnailStyle]}</div>
                        </div>
                    </div>
                ))}
            </div>
